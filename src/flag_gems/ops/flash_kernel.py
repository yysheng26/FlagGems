...
# ---------------------------------------------------------------------------
# FA3-style varlen forward kernel
#
# Differences from flash_varlen_fwd_kernel:
#   1. warp_specialize=True on the inner KV loop -> TMA + WGMMA pipeline on
#      Hopper (H800/H100). On older GPUs Triton ignores it gracefully.
#   2. IS_HOPPER constexpr: on Hopper + non-paged paths, use
#      tl.make_tensor_descriptor (device-side TMA) instead of make_block_ptr.
#   3. HAS_DESCALE constexpr: apply FP8 descale factors after QK^T and PV.
#   4. alibi intentionally not supported (FA3 contract).
# ---------------------------------------------------------------------------
...
def flash_varlen_fwd_fa3_kernel(
    ...
    qk_descale,   # scalar: q_descale * k_descale, pre-multiplied on host
    v_descale,    # scalar
    ...
    # ── masking steps (causal / SWA boundary blocks) ─────────────────────────
    n_block = n_block_max - 1
    for step in tl.range(0, n_masking_steps):
        col_idx = n_block * BLOCK_N + tl.arange(0, BLOCK_N)
        if is_paged:
