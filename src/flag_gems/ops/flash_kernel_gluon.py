"""
flash_kernel_gluon.py — Gluon flash attention forward for Hopper varlen/paged paths.
...
Phase 1 (this file): TMA loads + gl.dot (no warp_specialize yet).
Phase 2: Add gl.warp_specialize producer/consumer pipeline once correctness passes.
"""
...
def _softmax_step(
    acc, rowmax, rowsum,
    qk,                    # [BLOCK_M, BLOCK_N] fp32，layout = qk_layout
    scale_log2e,
...
def flash_varlen_fwd_gluon_kernel(
    ...
    Uses TMA for loads (runtime shape/strides OK — no TaskIdPropagation issue)
    and gl.dot for the attention matmuls. No warp_specialize in this phase.
    """
    ...
    # ── tile byte sizes (constexpr for mbarrier.expect) ─────────────────────
    q_nbytes:  gl.constexpr = _tile_nbytes(BLOCK_M, d,      dtype.primitive_bitwidth)
    k_nbytes:  gl.constexpr = _tile_nbytes(BLOCK_N, d,      dtype.primitive_bitwidth)
    kv_nbytes: gl.constexpr = k_nbytes * 2   # K + V together

    # ── WGMMA accumulator layouts ───────────────────────────────────────────
    # warps_per_cta: start from atom [4, 1] and expand.
    # Tile along M only when it won't broadcast (BLOCK_M > 16 * current_m_warps),
    # otherwise tile along N.  This matches the reference in 05-wgmma.py.
    ...
    # instr_shape n must divide BLOCK_N (or d) and satisfy n ≤ maxN.
    ...
    # rowmax / rowsum must have the SliceLayout derived from qk_layout (axis=1 reduction)
    row_layout: gl.constexpr = gl.SliceLayout(1, qk_layout)

    # ── TMA descriptors — runtime shape / ptr is fine in Gluon ─────────────
    ...
    # 05-wgmma.py:  store P → p_smem  →  p_smem.load(pv_a_layout).
    ...
    # ── load Q tile once ────────────────────────────────────────────────────
    mbarrier.expect(bar_q, q_nbytes)
    ...
    # ── accumulators ────────────────────────────────────────────────────────
    # Allocate o_smem_out early so acc can be initialized via smem.load(pv_layout).
    # This ensures the distributed layout of acc is compatible with smem.store(acc)
    # at epilogue (same smem → load → compute → store round-trip as in 05-wgmma.py).
    ...
    for n_block in range(n_block_max - 1, -1, -1):
        ...
        # async load K and V
        mbarrier.expect(bar_kv, kv_nbytes)
        ...
        mbarrier.wait(bar_kv, phase=phase)
        phase ^= 1
        fence_async_shared()

        # QK^T via WGMMA: Q_smem[BLOCK_M, d] × K^T_smem[d, BLOCK_N]
        kt_smem = k_smem.permute((1, 0))   # [d, BLOCK_N]
        ...
        # boundary mask: mask out-of-range K columns (last block may be partial)
        ...
        # online softmax — operates on NVMMADistributedLayout tensor
        acc, rowmax, rowsum, P = _softmax_step(
            acc, rowmax, rowsum, qk, scale_softmax_log2, BLOCK_M, BLOCK_N)

        # Store P to smem, then load it back as DotOperandLayout.
        # This is the only safe path: P has qk_layout which may differ from
        # pv_layout when BLOCK_N != d.  smem.load(pv_a_layout) gives the
        # correct register layout for warpgroup_mma LHS.
        P_typed = P.to(dtype)
        p_smem.store(P_typed)
        fence_async_shared()
        P_dot = p_smem.load(pv_a_layout)

        # PV via WGMMA: P_dot[BLOCK_M, BLOCK_N] × v_smem[BLOCK_N, d]
    ...
    # rowsum is SliceLayout(1, qk_layout); convert to pv's row layout for broadcast
    ...
    # write O via TMA store (o_smem_out allocated before loop for layout compatibility)
    o_smem_out.store(out)
    fence_async_shared()
    tma.async_copy_shared_to_global(desc_o, [m_block * BLOCK_M, 0], o_smem_out)
    tma.store_wait(pendings=0)

    # write LSE  shape [h, total_q] row-major
    # use standard layout for scalar operations
...
def flash_paged_fwd_gluon_kernel(
    ...
    # ── tile byte sizes ──────────────────────────────────────────────────────
    q_nbytes:  gl.constexpr = _tile_nbytes(BLOCK_M, d, dtype.primitive_bitwidth)
    kv_nbytes: gl.constexpr = _tile_nbytes(BLOCK_N, d, dtype.primitive_bitwidth) * 2
    ...
    # ── Q TMA descriptor (static per CTA) ───────────────────────────────────
    desc_q = tma.make_tensor_descriptor(
        q_seq, shape=[q_len, d], strides=[q_row_stride, 1],
        block_shape=[BLOCK_M, d], layout=q_layout)
    desc_o = tma.make_tensor_descriptor(
    ...
    # ── KV loop bounds ───────────────────────────────────────────────────────
    # k_bos is the global token offset; pages are indexed from 0.
    # token n (0-based within this batch) → page index in page_table:
    #   page_table[bid, n // block_size]
    # Within that page, the token lives at row: n % block_size
    # But because BLOCK_N == block_size, one BLOCK_N tile == one physical page.
    # The n_block index here counts pages relative to k_bos.
    ...
    # ── main attention loop ───────────────────────────────────────────────────
    for n_block in range(n_block_max - 1, -1, -1):
        # token offset within batch's K sequence
        tok_start = n_block * BLOCK_N   # first token in this tile (batch-local)

        # page_table[bid, tok_start // block_size]
        # Since BLOCK_N == block_size, tok_start // block_size == n_block
        pt_offset = bid * pt_batch_stride + n_block
        page_idx  = gl.load(page_table_ptr + pt_offset).to(gl.int32)

        # base pointer for this physical page, for the right KV head
        # k layout: [num_pages, block_size, hk, d]
        # k_ptr + page_idx * k_page_stride + kv_hid * k_head_stride
        ...
        # tokens remaining in this tile (for boundary mask)
        tile_tokens = gl.minimum(k_len - tok_start, BLOCK_N)

        # TMA descriptor: shape=[tile_tokens, d] but TMA needs static block_shape,
        # so we always load BLOCK_N rows and mask the excess later.
        desc_k = tma.make_tensor_descriptor(
            k_page_base, shape=[block_size, d],
            strides=[k_row_stride, 1],
            block_shape=[BLOCK_N, d], layout=kv_layout)
        desc_v = tma.make_tensor_descriptor(
            v_page_base, shape=[block_size, d],
            strides=[k_row_stride, 1],
            block_shape=[BLOCK_N, d], layout=kv_layout)
        ...
        # QK^T
        kt_smem = k_smem.permute((1, 0))
        qk = warpgroup_mma(q_smem, kt_smem,
                           gl.zeros((BLOCK_M, BLOCK_N), dtype=gl.float32, layout=qk_layout),
                           is_async=True, use_acc=False)
        qk = warpgroup_mma_wait(0, (qk,))

        # boundary mask (last tile may be partial)
        col_idx  = gl.arange(0, BLOCK_N)
        col_mask = col_idx[None, :] < tile_tokens
        qk = gl.where(col_mask, qk, float("-inf"))

        # causal mask: global col index = k_bos + tok_start + col_idx
        # global row index = q_bos + m_block*BLOCK_M + row_idx
        # causal: col_global <= row_global  (k_bos offset already in tok_start
        # via the k_len - q_len shift)
        if is_causal:
            row_limit = m_block * BLOCK_M + gl.arange(0, BLOCK_M) + (k_len - q_len)
        ...
        # P → smem → DotOperandLayout → PV WGMMA
        P_typed = P.to(dtype)
        p_smem.store(P_typed)
        fence_async_shared()
        P_dot = p_smem.load(pv_a_layout)
...
def _pick_paged_block_m(block_size: int, head_size: int,
    ...
    """Choose the largest BLOCK_M (power-of-two ≥ 32) such that the paged
    kernel's shared-memory footprint stays within *smem_limit* bytes.

    smem layout (conservative estimate, fp32 QK accumulator):
      Q  tile : BLOCK_M   × head_size × elem_bytes
      K  tile : block_size × head_size × elem_bytes
      V  tile : block_size × head_size × elem_bytes
      O  tile : BLOCK_M   × head_size × elem_bytes
      QK tile : BLOCK_M   × block_size × 4          (fp32)
    """
    for bm in (128, 64, 32):
        smem = (2 * bm + 2 * block_size) * head_size * elem_bytes + bm * block_size * 4
        if smem <= smem_limit:
            return bm
    raise RuntimeError(
        f"No valid BLOCK_M found for block_size={block_size}, "
        f"head_size={head_size}: even BLOCK_M=32 exceeds smem limit {smem_limit}"
    )
...
def flash_paged_fwd_gluon(
    ...
    if BLOCK_M == 0:
        BLOCK_M = _pick_paged_block_m(block_size, d, elem_bytes)
