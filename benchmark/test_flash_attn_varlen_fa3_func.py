...
class FlashAttnVarlenFa3Benchmark(base.Benchmark):
    ...
    def _make_input(self, config, dtype, device, use_gluon: bool = False):
        (
            cu_query_lens,
            seqused_k,
            nhead_q,
        ...
        return (
            ...
            None,            # k_descale
            None,            # v_descale
            {"fa_version": 3, "use_gluon": use_gluon},
        )
...
class _Fa3GluonBenchmarkMixin:
    """Mixin: 仿照底部注释的 gluon benchmark 结构，实现 use_gluon=False vs True 对比。
    自动过滤 gluon 不支持的 case（softcap / sliding window）。
    """

    def run(self):
        """Gems 内对比：fa3_triton (use_gluon=False) 作为 base，fa3_gluon (use_gluon=True) 作为测试。"""
        if Config.query:
        ...
        for dtype in self.to_bench_dtypes:
            ...
            for config in self.shapes:
                ...
                # baseline: use_gluon=False (triton fa3 路径)
                inp_t = self._make_input(config, dtype, self.device, use_gluon=False)
                args, kwargs = self.unpack_to_args_kwargs(inp_t)
                kwargs_t = {**kwargs, "fa_version": 3, "use_gluon": False}
                metric.shape_detail = self.record_shapes(*args, **kwargs_t)

                if "latency_base" in self.to_bench_metrics:
                    metric.latency_base = self.get_latency(
                        gems_op, *args, **kwargs_t
                    )
                _fa3_gluon_cleanup()

                # test: use_gluon=True (gluon 路径)
                inp_g = self._make_input(config, dtype, self.device, use_gluon=True)
                args_g, kwargs_g = self.unpack_to_args_kwargs(inp_g)
                kwargs_g = {**kwargs_g, "fa_version": 3, "use_gluon": True}
                ...
                if "speedup" in self.to_bench_metrics and metric.latency_base and metric.latency:
...
def _is_gluon_supported(config):
    """gluon 不支持 softcap 和 sliding window (local)"""
    # paged 格式: soft_cap 在 index -2, window 在 -1
    # nonpaged 格式: soft_cap 在 -2, window 在 -1
    soft_cap = config[-2]
    window = config[-1]
...
class FlashAttnVarlenFa3GluonBenchmark(_Fa3GluonBenchmarkMixin, FlashAttnVarlenFa3Benchmark):
    """paged 版本，使用最上面最多 case 的 FlashAttnVarlenFa3Benchmark，但去除不支持的 case。"""

    def set_shapes(self, shape_file_path=None):
        super().set_shapes(shape_file_path)
        self.shapes = [c for c in self.shapes if _is_gluon_supported(c)]


class FlashAttnVarlenFa3GluonNonPagedBenchmark(_Fa3GluonBenchmarkMixin, FlashAttnVarlenFa3NonPagedBenchmark):
    """nonpaged 版本，使用非 paged benchmark 的 case，但去除不支持的 case。"""
...
@pytest.mark.skipif(vendor_name == "cambricon", reason="Not supported")
@pytest.mark.flash_attn_varlen_func
def test_flash_attn_varlen_fa3_gluon_benchmark(monkeypatch):
    """全做：paged + nonpaged 两个版本。
    使用最上面 case 最多的版本，自动去除 gluon 不支持的 case（只有 G/H softcap/window，小部分）。
    仿照底部结构，case 来自未注释的最全版本。
    """
    monkeypatch.setenv("VLLM_CONFIGURE_LOGGING", "0")

    # paged gluon benchmark (filtered)
    bench_p = FlashAttnVarlenFa3GluonBenchmark(
        op_name="flash_attn_varlen_fa3_gluon_paged",
        torch_op=None,
        gems_op=flag_gems.ops.flash_attn_varlen_func,
        dtypes=[torch.float16, torch.bfloat16],
    )
    ...
    # nonpaged gluon benchmark (filtered)
    bench_np = FlashAttnVarlenFa3GluonNonPagedBenchmark(
        op_name="flash_attn_varlen_fa3_gluon_non_paged",
        torch_op=None,
        gems_op=flag_gems.ops.flash_attn_varlen_func,
        dtypes=[torch.float16, torch.bfloat16],
    )
